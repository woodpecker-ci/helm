export default {
  beforePrepare: async ({ exec, nextVersion }) => {
    ...
    await exec(
      `sed -i '' 's/^version:/version: ${nextVersion}/' charts/woodpecker/charts/agent/Chart.yaml`
    );
    await exec(
      `sed -i '' 's/^version:/version: ${nextVersion}/' charts/woodpecker/charts/server/Chart.yaml`
    );
    await exec(
